...
Fixpoint split {X Y : Type} (l : list (X*Y)) : (list X) * (list Y) :=
  match l with
  | nil => (nil, nil)
  | (x,y)::xs => let (xl, yl) := split xs in (x::xl, y::yl)
  end.
...
Lemma map_distr_app: forall (X Y: Type) (f: X -> Y) (l1: list X) (l2: list X),
...
Proof.
  intros. induction l as [|x xs rec].
  reflexivity.
  simpl. rewrite map_distr_app. rewrite rec. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros. induction l as [|x xs rec].
  reflexivity.
  simpl. unfold fold_map. rewrite <- rec. reflexivity.
Qed.
(* FILL IN HERE *)
...
Proof.
  intros. destruct p. unfold prod_uncurry. unfold prod_curry. reflexivity.
  Qed.
(** [] *)
...
Definition scc (n : cnat) : cnat := fun (X: Type) (f: X -> X) (x: X) => n X f (f x).
